...
inv_interest_simple = tk.Radiobutton(
    root, text="Simple", font=("Arial", 12), variable=inv_interest_var, value="Simple"
)
inv_interest_simple.grid(row=4, column=1, padx=10, pady=5, sticky="W")
inv_interest_compound = tk.Radiobutton(
    root,
    text="Compound",
    font=("Arial", 12),
    variable=inv_interest_var,
    value="Compound",
)
...
bond_value_label = tk.Label(
    root, text="Present value of the house:", font=("Arial", 12)
)
...
def calculate_bond():
    ...
    bond_rate = (
        float(bond_rate_entry.get()) / 100
    )  # Divide by 100 to convert percentage to decimal
    bond_time = int(bond_time_entry.get())

    monthly_interest_rate = bond_rate / 12
    num_payments = bond_time * 12

    bond_payment = (bond_value * monthly_interest_rate) / (
...
inv_button = tk.Button(
    root, text="Calculate Investment", font=("Arial", 12), command=calculate_investment
)
inv_button.grid(row=11, column=0, padx=10, pady=10)

bond_button = tk.Button(
    root, text="Calculate Bond", font=("Arial", 12), command=calculate_bond
)
